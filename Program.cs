...
/*
        ...
        CollectionItem.UpdateItemByName(item1.ItemName, "Så som i himmelen", "En rörande dramakomedi om en berömd dirigent som återvänder till sin hemby och förändrar livet för invånarna där", 100, ItemCondition.Decent, "Fodralet saknas.");
        ...
        CollectionItem.UpdateItemById(item2.ItemId, "Blade Runner", "En uppföljare till den klassiska Blade Runner, som utforskar frågor om mänsklighet och identitet i en dystopisk framtid.", 50, ItemCondition.Poor, "DVD-skivan är repad.");
        ...
        CollectionItem.EstimatedValue(item1.ItemId, newValue);
        foreach (var item in CollectionItem.GetAllItems())
        {
            ShowAllItemInfo(item);
        }
